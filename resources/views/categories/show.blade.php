...
@section('content')
  <main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
    <x-breadcrumb :items="[['label' => '分类', 'url' => route('notes.index')], ['label' => $category->name]]" />
    {{-- 页头 --}}
    <div class="flex items-center justify-between mb-8">
      <div>
        <p class="text-xs font-medium tracking-[0.2em] text-primary uppercase mb-2">Category</p>
        <h1 class="text-2xl sm:text-3xl font-bold tracking-tight">{{ $category->name }}</h1>
      </div>
            ...
            @if ($note->tags->isNotEmpty())
              <div class="flex flex-wrap items-center gap-2 text-xs text-text-secondary">
                @foreach ($note->tags->take(3) as $tag)
                  <a href="{{ route('tags.show', $tag) }}" class="hover:text-primary transition" onclick="event.stopPropagation()">{{ $tag->name }}</a>
                @endforeach
              </div>
            @endif
          </div>
    ...
    @empty
      <x-empty-state icon="article" title="该分类下还没有文章" description="换个分类看看？" />
    @endforelse
