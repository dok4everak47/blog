...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
  <div class="flex gap-10">
    ...
    <div class="flex-1 min-w-0">
      ...
      <?php if($about['opening']): ?>
      <div class="mb-8">
        <p class="text-[15px] leading-[1.85] text-text-secondary"><?php echo e($about['opening']); ?></p>
      </div>
      <?php endif; ?>

      
      <?php if(!empty($about['intro_items'])): ?>
      <section class="mb-8" id="about-intro">
        ...
        <dl class="space-y-3">
          <?php $__currentLoopData = $about['intro_items']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="flex items-baseline gap-3 py-2 border-b border-dashed border-border last:border-0">
            <dt class="flex items-center gap-1.5 text-sm font-medium text-text min-w-[100px]">
              <span><?php echo e($item['emoji']); ?></span> <?php echo e($item['label']); ?>

            </dt>
            <dd class="text-sm text-text-secondary"><?php echo e($item['value']); ?></dd>
          </div>
      ...
      <?php if(!empty($about['tech_interests'])): ?>
      <section class="mb-8" id="about-tech">
        <h2 class="text-lg font-bold text-text mb-4 flex items-center gap-2">
          <span class="text-primary">◆</span> 技术兴趣
        </h2>
        <div class="flex flex-wrap gap-2">
          <?php $__currentLoopData = $about['tech_interests']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <span class="rounded-lg bg-surface-2 border border-border px-3 py-1.5 text-xs font-medium text-text-secondary"><?php echo e($tag); ?></span>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
      </section>
      <?php endif; ?>
      ...
      <?php if(!empty($about['contacts'])): ?>
      <section class="mb-8" id="about-contact">
        <h2 class="text-lg font-bold text-text mb-4 flex items-center gap-2">
          <span class="text-primary">◆</span> 联系方式
        </h2>
        <dl class="space-y-3">
          <?php $__currentLoopData = $about['contacts']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <div class="flex items-baseline gap-3 py-2 border-b border-dashed border-border last:border-0">
            <dt class="text-sm font-medium text-text min-w-[80px]"><?php echo e($item['type']); ?></dt>
            <dd class="text-sm text-text-secondary"><?php echo e($item['value']); ?></dd>
          </div>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </dl>
      ...
      <?php if(empty($about['opening']) && empty($about['intro_items']) && empty($about['tech_interests']) && empty($about['contacts'])): ?>
